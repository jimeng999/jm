"""
计费服务 (BYOK 版本)
支持 BYOK 模式：用户 Key 零差价，平台 Key 加价 30%
"""
...
class BillingService:
    ...
    # BYOK 模式加成比例
    BYOK_PLATFORM_MARKUP = 1.30  # 平台 Key 加价 30%
    ...
    def calculate_price(
        ...
        if key_source == "user_byok":
            # 用户自己的 Key：零差价
            price = 0.0
        elif key_source == "deepseek_trial":
            # DeepSeek 免费体验：免费
            price = 0.0
        elif key_source == "platform":
            # 平台 Key：加价 30%
            price = cost["cost_cny"] * self.BYOK_PLATFORM_MARKUP
        else:
            # 默认：加价 30%
            price = cost["cost_cny"] * self.BYOK_PLATFORM_MARKUP
        ...
        return {
            **cost,
            "price_cny": price,
            "price_usd": price / self.token_rate if self.token_rate > 0 else 0,
    ...
    def record_api_usage(
        ...
        """记录 API 用量"""
        # 计算成本和收入
        pricing = self.calculate_price(model, input_tokens, output_tokens, is_byok, key_source)
        ...
        record = usage_manager.record_usage(
            ...
            cost=cost,
            revenue=revenue,
            metadata={"is_byok": is_byok, "key_source": key_source}
        )
        
        # 扣费（仅在有费用时扣费）
        if revenue > 0:
            if not self.charge_user(user_id, revenue, f"{endpoint} - {model} ({key_source})"):
